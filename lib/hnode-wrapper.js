...
const config = require(path.join(__dirname, '..', 'rythmus.config.json'))
const Point = require(path.join(__dirname, 'utils', 'point'))

const server = new hnode.Server()

server.on('newnode', node => node.lockRate(1000 / 40))
